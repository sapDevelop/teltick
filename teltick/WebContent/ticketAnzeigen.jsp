	...
	<form action="TicketVerwaltungController" method="post" onreset="fenster_schliessen('fenster_<c:out value="${ param.id }" />');" name="form_<c:out value="${param.id}" />" id="form_<c:out value="${param.id}" />" onsubmit="return submitUmleiten(this, '<c:out value="${param.id}" />');" >
		...
		<c:if test="${Ticket.ticketId != -1}">
			<c:choose>
				...
				<c:otherwise>
					<div class="ueberschrift_content_TicketAnzeigen_Neu">
						<h3>
							Ticket: #<c:out value="${Ticket.ticketId }"></c:out>
						</h3>
					</div>
				</c:otherwise>
			</c:choose>
		...
		<div class="inhalt_contentTicketAnzeigen inhalt_contentTicketAnzeigen_beschreibung">
			...
			<div class="inhalt_contentTicketAnzeigen contentTicketAnzeigen_inlineBlock">
				<c:out value="${Zuweisung.mitarbeiter.vorname}"></c:out> <c:out value="${Zuweisung.mitarbeiter.name}"></c:out>
			</div>
		<div class="button_ticket_anzeigen">
		
		<c:choose>
			<c:when test="${requestScope.RechtTicketvorschau}">
				<input type="submit" value="Neues Ticket" class="" title="Neues Ticket" onclick="oeffne_fenster(1); fenster_schliessen('fenster_<c:out value="${ param.ajax_id}" />');"/>
			</c:when>
			<c:otherwise>
		...
		<input type="button" value="Fenster schließen" class="" title="Fenster schließen" onclick="fenster_schliessen('fenster_<c:out value="${ param.ajax_id}" />');"/>
		</div>
